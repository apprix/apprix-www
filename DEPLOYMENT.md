...
| Muuttuja | Tila | Huom |
|---|---|---|
| `APP_URL` | `https://www3.apprix.fi` | Päivitettävä `https://www2.apprix.fi` domain-siirron yhteydessä |
...
Hallintapaneeli löytyy osoitteesta: `https://www3.apprix.fi/hallinta` (myöhemmin `https://www2.apprix.fi/hallinta`)
...
| www2.apprix.fi | 62.238.10.202 | Tuleva domain — lisättävä DNS + SSL |
| apprix.fi | 95.216.201.18 | Vanha palvelin |
| www.apprix.fi | 95.216.201.18 | Vanha palvelin |

### Domain-siirto www3 → www2 (kun aika tulee)

TTL:n lyhentäminen ennen muutoksia: aseta TTL 300s Hetzner DNS -konsolissa, odota vanhan TTL:n verran, tee muutos.

```bash
# 1. Lisää www2.apprix.fi DNS-tietue Hetzner DNS:ään osoitteeseen 62.238.10.202

# 2. Lisää www2.apprix.fi SSL-sertifikaattiin
certbot --nginx -d www2.apprix.fi -d www3.apprix.fi

# 3. Päivitä APP_URL palvelimen .env:iin
APP_URL=https://www2.apprix.fi

# 4. Päivitä statamic.com-sivuston domain
# → https://statamic.com/account/sites → klikkaa sivustoa → vaihda URL → www2.apprix.fi

# 5. Poista basic auth Nginx-konfiguraatiosta
```
...
- [ ] **Custom roolit käyttöön** — Korjaantuu automaattisesti domain-siirron jälkeen kun APP_URL on `www2.apprix.fi` ja statamic.com-domain täsmää
